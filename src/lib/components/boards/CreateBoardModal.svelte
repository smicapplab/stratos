		...
		<div role="dialog" aria-modal="true" aria-labelledby="modal-title" class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
			...
			<form method="POST" action="/boards?/create" use:enhance={() => {
				...
				<div class="space-y-2">
					...
					<input type="text" name="name" id="name" required placeholder="e.g. Q3 Sprint Planning" bind:value={newBoardName} oninput={handleNameChange} class="w-full px-4 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary transition-all text-zinc-900 dark:text-white" />
				</div>
				
				<div class="space-y-2">
					<label for="prefix" class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Prefix</label>
					<input type="text" name="prefix" id="prefix" required maxlength="10" placeholder="e.g. Q3SP" value={newBoardPrefix} oninput={handlePrefixInput} class="w-full px-4 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700 focus:ring-brand-primary focus:border-brand-primary rounded-lg focus:outline-none focus:ring-2 transition-all uppercase text-zinc-900 dark:text-white font-mono" />
				</div>
				
				<div class="space-y-2">
					<label for="projectId" class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Assign to Project</label>
					<select name="projectId" id="projectId" bind:value={currentProjectId} class="w-full px-4 py-2 bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-primary transition-all text-zinc-900 dark:text-white" required>
