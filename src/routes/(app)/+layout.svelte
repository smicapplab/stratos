<script lang="ts">
	...
	onMount(() => {
		if (
			!localStorage.getItem("stratos-theme") &&
			data.group?.defaultTheme
		) {
...
<div
	class="flex h-screen bg-zinc-50 dark:bg-zinc-950 overflow-hidden font-sans text-zinc-900 dark:text-zinc-100"
>
	<!-- Sidebar / Workspace Shell -->
	<aside
		class="hidden lg:flex w-64 flex-shrink-0 flex-col bg-white/70 dark:bg-zinc-900/50 backdrop-blur-xl border-r border-zinc-200/80 dark:border-zinc-800/80 z-20 transition-all duration-300 ease-in-out {isSidebarOpen
			? 'ml-0'
			: '-ml-64'}"
	>
		<!-- Brand Header -->
		<div
			class="h-16 flex items-center px-6 border-b border-zinc-200/50 dark:border-zinc-800/50"
		>
		...
		<!-- Navigation -->
		<nav
			class="flex-1 overflow-y-auto py-4 px-3 space-y-6 custom-scrollbar"
		>
			<!-- Top Nav -->
			<div>
				<ul class="space-y-1">
			...
			<div>
				<div
					class="px-3 mb-2 flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 group"
				>
					Projects
					<button
				...
				<ul class="space-y-2">
					<!-- We replaced the inline project form with CreateProjectModal at the bottom -->
					{#each projects as project (project.id)}
						<li>
							<div
								class="flex items-center px-2 py-1 w-full rounded-md text-[13px] font-semibold text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 transition-colors group relative"
							>
								<div
								...
								>
									...
									<a
										...
										class="flex-1 truncate hover:text-zinc-900 dark:hover:text-white flex items-center gap-2"
									>
										<DynamicIcon
											name={project.icon}
									...
									{#if user?.role === "Admin"}
										<button
											...
											onclick={(e) => {
												...
												selectedProjectIdForBoard =
													project.id;
												collapsedProjects[project.id] =
													false; // Expand the project
											}}
										>
											<Plus class="w-3.5 h-3.5" />
										</button>
							...
							{#if !collapsedProjects[project.id]}
								<ul
									class="mt-1 space-y-0.5 ml-4 border-l border-zinc-200 dark:border-zinc-800 pl-2"
								>
									{#each boardsByProject[project.id] || [] as board (board.id)}
										<li>
											...
											>
												<DynamicIcon
													name={board.icon}
													class="w-3.5 h-3.5 transition-transform group-hover:scale-110 {currentPath ===
													`/boards/${board.id}`
									...
									{#if !(boardsByProject[project.id] && boardsByProject[project.id].length > 0) && addingBoardToProject !== project.id}
										<li
											class="px-3 py-1.5 text-[12px] text-zinc-400 italic"
										>
											No boards
										</li>
									{/if}
			...
			{#if user?.role === "Admin"}
				<div>
					<div
						class="px-3 mb-2 text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400"
					>
						Administration
					</div>
					<ul class="space-y-1">
		...
		<!-- User Footer -->
		<div
			class="p-4 border-t border-zinc-200/50 dark:border-zinc-800/50 relative user-menu-container"
		>
			{#if isUserMenuOpen}
				<div
			...
			>
				<div
					class="w-8 h-8 rounded-full bg-gradient-to-tr from-emerald-400 to-cyan-500 flex items-center justify-center text-white text-xs font-bold shadow-inner flex-shrink-0"
				>
					{user?.name?.charAt(0).toUpperCase()}
				</div>
				<div class="truncate flex-1">
					<div
						class="text-sm font-medium truncate text-zinc-900 dark:text-zinc-100"
					>
						{user?.name}
					</div>
					<div
				...
				<ChevronRight
					class="w-4 h-4 text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-300 transition-transform duration-200 {isUserMenuOpen
						? '-rotate-90'
						: 'rotate-90'}"
				/>
			</button>
	...
	<div class="flex-1 flex flex-col relative z-10 overflow-hidden">
		<!-- Top Navbar -->
		<header
			class="h-16 bg-white/40 dark:bg-zinc-950/40 backdrop-blur-md border-b border-zinc-200/50 dark:border-zinc-800/50 flex items-center justify-between px-8 sticky top-0 z-30 transition-all duration-300"
		>
			<!-- Left Side Actions -->
			<div class="flex items-center gap-4 flex-1 max-w-xl">
				...
				<div class="relative group flex-1">
					<Search
						class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400 group-focus-within:text-brand-primary transition-colors"
					/>
					<input
						type="text"
						readonly
						...
						class="w-full bg-zinc-100/50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-full pl-10 pr-4 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary/50 focus:border-brand-primary transition-all placeholder:text-zinc-500 cursor-pointer"
					/>
				</div>
			</div>
			...
			<div class="flex items-center gap-2 ml-4">
				...
				<div class="relative">
					...
					{#if isNotificationsOpen}
						...
						></div>
						<div
							class="absolute top-full right-0 mt-2 w-80 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-xl z-50 overflow-hidden max-h-[60vh] flex flex-col"
						>
							<div
								class="flex items-center justify-between px-4 py-3 border-b border-zinc-200 dark:border-zinc-800"
							>
								<h3
								...
								{#if unreadCount > 0}
									<button
										class="text-xs font-medium text-brand-primary hover:text-brand-primary dark:text-brand-primary"
							...
							<div class="overflow-y-auto flex-1">
								...
								{:else}
									{#each notifications.slice(0, 20) as notif (notif.id)}
										...
										<a
											href={taskUrl}
											class="w-full text-left px-4 py-3 border-b border-zinc-100 dark:border-zinc-800/50 hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors block {notif.readAt
												? 'opacity-60'
												: ''}"
											onclick={() => {
										...
										>
											<div class="flex items-start gap-2">
												{#if !notif.readAt}
													<div
														class="w-2 h-2 rounded-full bg-brand-primary/100 mt-1.5 flex-shrink-0"
													></div>
												{:else}
													<div
