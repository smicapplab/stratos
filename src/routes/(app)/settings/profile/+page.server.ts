...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const [user] = await db.select({ id: users.id, email: users.email, name: users.name, avatarUrl: users.avatarUrl, jobTitle: users.jobTitle, theme: users.theme }).from(users).where(
