<script lang="ts">
	...
	let projectMembersList = $derived(
	...
	function getActionText(log: any) {
		const name = `<span class="font-semibold text-zinc-900 dark:text-zinc-100">${log.userName}</span>`;
		...
		switch (log.actionType) {
			...
			case "member_joined":
				return `${name} invited a member as <span class="font-semibold text-zinc-900 dark:text-zinc-100">${details.role}</span>`;
			case "member_removed":
				return `${name} removed a member`;
			case "tag_created":
				return `${name} created tag <span class="px-1.5 py-0.5 rounded text-xs border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-black">${details.name}</span>`;
			case "tag_updated":
				return `${name} updated tag <span class="px-1.5 py-0.5 rounded text-xs border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-black">${details.name}</span>`;
			case "tag_deleted":
				return `${name} deleted a tag`;
			case "board_created":
				return `${name} created board <span class="font-semibold text-zinc-900 dark:text-zinc-100">${details.name}</span>`;
			case "board_deleted":
				return `${name} deleted a board`;
			default:
				return `${name} performed ${log.actionType}`;
		}
	}
	...
	async function loadMoreActivity() {
		...
		try {
			const res = await fetch(
				`/api/projects/${project.id}/activity?offset=${allActivity.length}&limit=15`,
			);
			if (res.ok) {
...
<div
	class="max-w-4xl mx-auto p-4 sm:p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500"
>
	<div
		class="flex items-center gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-6"
	>
		<a
			href="/"
			class="p-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors text-zinc-500"
		>
		...
		<div>
			...
			<p class="text-zinc-500 text-sm mt-1">
				Manage visibility, access, and team members for this project.
			</p>
		</div>
	</div>
	...
	{#if form?.error}
		<div
			class="p-4 bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-900 rounded-xl text-sm font-medium"
		>
			{form.error}
		</div>
	{/if}
	...
	<div class="space-y-8">
		...
		>
			...
			>
				<div class="space-y-3">
					<label class="flex items-start gap-3 cursor-pointer group">
						...
						<div>
							...
							>
								Everyone in the group can view tasks and boards.
							</div>
						</div>
					</label>
				...
				{#if isProjectAdmin}
					<button
						type="submit"
						class="w-auto px-6 py-2 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-900 dark:text-zinc-100 text-sm font-semibold rounded-lg transition-colors mt-4"
					>
						Save Visibility
					</button>
				{/if}
		...
		>
			<div
				class="p-5 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between"
			>
				<div>
			...
			{#if isProjectAdmin}
				<div
					class="p-5 bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800"
				>
					<form
						method="POST"
						action="?/addMember"
			...
			{#if projectAdmins.length > 0}
				<div
					class="px-5 py-2 bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800 text-xs font-bold uppercase tracking-wider text-zinc-500"
				>
					Project Admins
				</div>
				<ul class="divide-y divide-zinc-100 dark:divide-zinc-800/50">
					{#each projectAdmins as member}
						<li
							class="flex items-center justify-between p-4 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors"
						>
							<div class="flex items-center gap-3">
								...
								<div>
									<div
										class="text-sm font-semibold text-zinc-900 dark:text-zinc-100 flex items-center gap-2"
									>
										{member.name}
										<span
			...
			{#if projectMembersList.length > 0}
				<div
					class="px-5 py-2 bg-zinc-50 dark:bg-zinc-900/50 border-y border-zinc-200 dark:border-zinc-800 text-xs font-bold uppercase tracking-wider text-zinc-500"
				>
					Project Members
				</div>
				<ul class="divide-y divide-zinc-100 dark:divide-zinc-800/50">
					{#each projectMembersList as member}
						...
						>
							<div class="flex items-center gap-3">
								<div
									class="w-9 h-9 rounded-full bg-gradient-to-tr from-zinc-300 to-zinc-400 dark:from-zinc-700 dark:to-zinc-600 flex items-center justify-center text-white text-sm font-bold shadow-inner"
								>
									{member.name.charAt(0).toUpperCase()}
								</div>
								<div>
	...
	<div class="md:col-span-2 space-y-8 mt-6">
		...
		>
			...
			>
				<div>
					...
					<p class="text-sm text-zinc-500">
						Manage tags for tasks within this project.
					</p>
				</div>
			</div>
				...
				>
					...
					<div class="w-32 flex flex-col gap-1.5">
						...
						<div class="relative">
							...
							<div
								class="absolute inset-y-0 right-2 flex items-center pointer-events-none text-zinc-500"
							>
								<svg
									class="w-4 h-4"
			...
			<ul class="divide-y divide-zinc-100 dark:divide-zinc-800/50">
				{#each data.tags.filter((t) => !t.deletedAt) as tag}
					{#if editTagId === tag.id}
						<li
							class="p-4 bg-zinc-50 dark:bg-zinc-900 flex flex-col sm:flex-row items-start sm:items-center gap-3"
						>
							<input
								type="text"
								bind:value={editTagName}
								class="flex-1 bg-white dark:bg-black border border-zinc-300 dark:border-zinc-700 rounded px-3 py-1.5 text-sm w-full"
							/>
							<div
							...
							>
								...
								<button
									...
									class="px-3 py-1.5 text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 text-sm font-semibold whitespace-nowrap"
									>Cancel</button
								>
							</div>
						</li>
					{:else}
						<li
							class="flex items-center justify-between p-4 hover:bg-zinc-50/50 dark:hover:bg-zinc-800/20 transition-colors group"
						>
							<div class="flex items-center gap-3">
								<div
									class="px-2 py-1 text-xs font-bold uppercase tracking-wider rounded-md bg-{tag.color}-500/10 text-{tag.color}-600 dark:text-{tag.color}-400 border border-{tag.color}-500/20"
								>
									{tag.name}
								</div>
							</div>
			...
			{#if data.tags.filter((t) => !t.deletedAt).length === 0}
				<div class="p-8 text-center text-zinc-500 text-sm">
		...
		>
			<div
				class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/20 flex items-center gap-3"
			>
			...
			<div class="p-6">
				{#if allActivity.length === 0}
					<div
						class="text-center py-12 text-zinc-500 dark:text-zinc-400 text-sm"
					>
						No activity recorded yet.
					</div>
				{:else}
					<div
						class="relative before:absolute before:inset-y-0 before:left-[17px] before:w-[2px] before:bg-zinc-200 dark:before:bg-zinc-800/80"
					>
						{#each allActivity as log}
							{@const Icon = getActionIcon(log.actionType)}
							<div
								class="relative flex items-start gap-4 mb-2 last:mb-0"
							>
								<!-- Timeline Node -->
								<div
									class="relative z-10 flex-shrink-0 w-9 h-9 rounded-full bg-white dark:bg-black border-2 border-zinc-200 dark:border-zinc-800 flex items-center justify-center"
								>
									<Icon
								...
								<div class="flex-1 pt-1.5 pb-2">
									...
									<div
										class="mt-1 text-xs text-zinc-500 dark:text-zinc-500"
									>
					...
					{#if hasMore}
						<div class="mt-6 flex justify-center pb-4">
							<button
								class="px-4 py-2 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-900 dark:text-zinc-100 text-sm font-semibold rounded-lg transition-colors disabled:opacity-50"
